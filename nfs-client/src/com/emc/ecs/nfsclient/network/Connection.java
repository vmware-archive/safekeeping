...
public class Connection {
	...
	/**
	 * internal sending queue (not tcp sending buffer) the rename/lookup/readdir has
	 * size between 256-516bytes, 5M can hold 10000 pending requests for data
	 * writing, each request is 512K and maximum 40 pending requests are allowed.
	 * (512k is the preferred size in a single request. For object > 512K, it should
	 * be split to 512K-size chunks.) the queue size need be 20M.
	 */
	...
	/**
	 * <ul>
	 * <li>If <code>true</code>, use a privileged local port (below 1024) for RPC
	 * communication.</li>
	 * <li>If <code>false</code>, use any non-privileged local port for RPC
	 * communication.</li>
	 * </ul>
	 */
	...
	/**
	 * Store the ChannelFuture instances while they are in progress. The map is
	 * final, but the content will change.
	 */
	private final ConcurrentHashMap<Integer, ChannelFuture> _futureMap = new ConcurrentHashMap<Integer, ChannelFuture>();

	/**
	 * Store the Xdr response instances while they are in progress. The map is
	 * final, but the content will change.
	 */
	...
	/**
	 * @param remoteHost        A unique name for the host to which the connection
	 *                          is being made.
	 * @param port              The remote host port being used for the connection.
	 * @param usePrivilegedPort
	 *                          <ul>
	 *                          <li>If <code>true</code>, use a privileged port
	 *                          (below 1024) for RPC communication.</li>
	 *                          <li>If <code>false</code>, use any non-privileged
	 *                          port for RPC communication.</li>
	 *                          </ul>
	 */
	 ...
	 * function will not retry. It is the responsibility of the application to do
	 * retry. The behaviours: a. If the tcp connection is not established yet: (1).
	 * sendAndWait will wait until the connection is established or timeout or
	 * network error occurs. (2). Once the connection is established, sendAndWait
	 * can send data. b. If the tcp connection is established: (1). the request is
	 * put in the internal queue of netty. Netty will send it asap. If there are
	 * already lots of pending requests in the queue, sendAndWait return error. (2).
	 * sendAndWait will wait until it gets a response from NFS server or timeout.
	 * (3). If the tcp connection is broken, the function return error with network
	 * error. c. If the tcp connection is broken suddenly: (1) the old sendAndWait
	 * will get the network error or timeout (2) The new sendAndWait will follow
	 * (a).
	 * 
	...
	public Xdr sendAndWait(int timeout, Xdr xdrRequest) throws RpcException {
		...
		if (timeoutFuture.isSuccess() == false) {
			...
			LOG.warning("cause: " + timeoutFuture.getCause());
	...
	protected void connect() throws RpcException {
		...
		LOG.fine(String.format("%s connecting to %s", logPrefix, getRemoteAddress()));
		// }
	...
	/**
	 * Update the response map with the response and notify the thread waiting for
	 * the response. Do nothing if the future has been removed.
	 * 
	 * @param xid
	 * @param response
	 */
	 ...
	 * 
	 * <p>
	 * Some NFS servers apparently may require that some requests originate on an
	 * Internet port below IPPORT_RESERVED (1024). This is generally not used,
	 * though, as the client then has to run as a user authorized for privileged,
	 * which is dangerous. It is also not generally needed.
	 * </p>
	 * 
	 * @return
	 *         <ul>
	...
	private Channel bindToPrivilegedPort() throws RpcException {
		LOG.info("Attempting to use privileged port.");
		for (int port = 1023; port > 0; --port) {
			try {
				...
				if (bindFuture.isSuccess()) {
					LOG.info("Success! Bound to port " + port);
