...
public class RecordMarkingUtil {
	...
	/**
	 * The size mask is applied to the long fragment size to get the real unsigned
	 * int size. When this mask is applied to <code>LAST_FRAG</code>, the resulting
	 * real fragment size is 0. For all real fragment sizes, this mask has no
	 * effect.
	 */
	private static final int SIZE_MASK = 0x7fffffff;

	/**
	 * RFC suggest setting the record size to MTU (Ethernet: MTU=1500 - 40(ip and
	 * tcp header). But, When we send multiple fragments continuously, some NFS
	 * server kill the connection and the report the error below: "RPC: multiple
	 * fragments per record not supported" To bypass this limitation, set a big
	 * MTU_SIZE number now.
	 */
	 ...
	 * The usual logger.
	 */
	private static final Logger LOG = Logger.getLogger(RecordMarkingUtil.class.getName());
		...
		for (ByteBuffer buffer : buffers) {
			...
			if (bytesToWrite + buffer.remaining() > MTU_SIZE) {
				if (outBuffers.isEmpty()) {
					LOG.warning("too big single byte buffer " + buffer.remaining());
	...
	/**
	 * @param fragmentSize A long that contains either the int number of bytes in
	 *                     the fragment, or a special value if this is the last
	 *                     fragment.
