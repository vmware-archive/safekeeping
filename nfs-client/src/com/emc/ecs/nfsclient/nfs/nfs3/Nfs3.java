...
public class Nfs3 implements Nfs<Nfs3File> {
     ...
     * The usual logger.
     */ 
    private static final Logger LOG = Logger.getLogger(Nfs3.class.getName());
     ...
     */
    public static int NFS_TIMEOUT = 10; // unit: seconds
    ...
    /**
     * Use port number <1024
     */
    private  boolean _usePrivilegedPort  ;
    /**
     ...
     * @throws IOException
     */
    public Nfs3(String server, String exportedPath, byte[] rootFileHandle, int maximumRetries,boolean usePrivilegedPort) throws IOException {
        this(server, exportedPath, rootFileHandle, null, maximumRetries,usePrivilegedPort);
    }
     ...
     * @throws IOException 
     */
    public Nfs3(String server, String exportedPath, Credential credential, int maximumRetries ,boolean usePrivilegedPort) throws IOException {
        this(server, exportedPath, null, credential, maximumRetries,usePrivilegedPort);
    }
     ...
     * @throws IOException
     */
    public Nfs3(String absolutePath, int uid, int gid, int maximumRetries,boolean usePrivilegedPort) throws IOException {
        this(absolutePath, new CredentialUnix(uid, gid, null), maximumRetries,usePrivilegedPort);
    }
     ...
     * @throws IOException
     */
    public Nfs3(String absolutePath, Credential credential, int maximumRetries,boolean usePrivilegedPort) throws IOException {
        this(getServer(absolutePath), getExportedPath(absolutePath), credential, maximumRetries,usePrivilegedPort);
    }
     ...
     * @throws IOException
     */
    public Nfs3(String server, String exportedPath, byte[] rootFileHandle, Credential credential, int maximumRetries,boolean usePrivilegedPort) throws IOException {
 ...
 //       boolean usePrivilegedPort = false;
        for (int i = 0; i < MOUNT_MAX_RETRIES; ++i) {
            try {
                ...
                if (_usePrivilegedPort) {
                    LOG.warning("Mounting with privileged port - attempt with unprivileged failed with an authentication error.");
                }
                response.unmarshalling(NetMgr.getInstance().sendAndWait(_server, portOfMountService, _usePrivilegedPort, mountXdr, MOUNT_RPC_TIMEOUT));
            	...
            	_usePrivilegedPort = handleRpcException(e, i);
            }
        }
        ...
        for (int i = 0; i < MOUNT_MAX_RETRIES; ++i) {
            try {
                ...
                NetMgr.getInstance().sendAndWait(_server, portOfMountService, _usePrivilegedPort, unmountXdr, MOUNT_RPC_TIMEOUT);
            } catch (RpcException e) {
                if (i+1 < MOUNT_MAX_RETRIES) {
                    LOG.warning(String.format(
                            "unmount failure, server: %s, export: %s, nfs version: %s\n", _server,
                            _exportedPath, VERSION)+  e.getMessage());
                }
            }
        }
        ...
        if (LOG.getLevel()==Level.FINE) {
            ...
            LOG.fine(String.format( "nfs server %s:%s supports auth mode %s", _server, _exportedPath,
    ...
    private boolean handleRpcException(RpcException e, int attemptNumber)
        ...
        LOG.warning("retry " + (attemptNumber + 1));
