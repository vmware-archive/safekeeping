...
public class NfsWriteResponse extends NfsResponseBase {
	...
	private static final Logger _log = Logger.getLogger(NfsWriteResponse.class.getName());
	 ...
	 * <li>FILE_SYNC = 2 - Committed all data and metadata to stable storage before
	 * returning.</li>
	 * </ul>
	 */
	...
	/**
	 * The number of bytes of data written to the file. The server may write fewer
	 * bytes than requested. If so, the actual number of bytes written starting at
	 * location, <code>offset</code>, is returned.
	 */
	private int _count;
	...
	/**
	 * This is a cookie that the client can use to determine whether the server has
	 * changed state between a call to WRITE and a subsequent call to either WRITE
	 * or COMMIT. This cookie must be consistent during a single instance of the NFS
	 * version 3 protocol service and must be unique between instances of the NFS
	 ...
	 * </p>
	 * 
	 * @param nfsVersion The NFS version number. This is ignored for now, as only
	 *                   NFSv3 is supported, but is included to allow future support
	 *                   for other versions.
	 */
	...
	public void unmarshalling(Xdr xdr) throws RpcException {
		...
		if (stateIsOk()) {
			...
			_verf = xdr.getLong();
			_log.fine(String.format("committed %s verf %s", _committed, _verf));
		}
	}
	...
	/**
	 * @return The number of bytes of data written to the file. The server may write
	 ...
	 *         call to either WRITE or COMMIT. This cookie must be consistent during
	 *         a single instance of the NFS version 3 protocol service and must be
