...
public class RpcWrapper<S extends NfsRequestBase, T extends NfsResponseBase> {
     ...
     * The usual logger.
     */ 
    private static final Logger LOG = Logger.getLogger(RpcWrapper.class.getName());
    /**
    ...
    public String chooseIP(byte[] key) throws IOException {
        if (_ips == null || _ips.length == 0) {
            if (_server != null) {
                LOG.warning("ip list is not initialized, fallback to server");
                return _server;
            }
    ...
    private void callRpcChecked(S request, RpcResponseHandler<? extends T> responseHandler, String ipAddress)
            throws IOException {if (LOG.getLevel()==Level.FINE) {
        LOG.fine(String.format("server %s, port %d, request %s", _server, _port, request));
            }
        callRpcNaked(request, responseHandler.getNewResponse(), ipAddress);

        if (LOG.getLevel()==Level.FINE) {
            LOG.fine(String.format("server %s, port %d, request %s", _server, _port, responseHandler.getResponse()));
        }
    ...
    private void handleRpcException(RpcException e, int attemptNumber) throws IOException {
        ...
        if (!(e.getStatus().equals(RpcStatus.NETWORK_ERROR))) {
            ...
            if (attemptNumber + 1 < _maximumRetries) {
                ...
                LOG.warning(String.format("network error happens, server %s, attemptNumber %d",   _server, attemptNumber) );
                return;
            }
