...
public class ConcurrentDoublyLinkedList<E extends Serializable> extends AbstractCollection<E> implements Serializable {
    ...
    final class CLDIterator implements Iterator<E> {
        ...
        private Node<E> next = ConcurrentDoublyLinkedList.this.header.forward();
        ...
        @Override
        public E next() {
            final Node<E> l = this.last = this.next;
            if (l == null) {
     ...
     * @param v the element
     */
    private static void checkNullArg(final Object v) {
        if (v == null) {
    ...
    public ConcurrentDoublyLinkedList() {
        final Node h = new Node(null, null, null);
        final Node t = new Node(null, null, h);
        h.setNext(t);
        this.header = h;
        this.trailer = t;
    }
     ...
     *                              <tt>null</tt>
     */
    public ConcurrentDoublyLinkedList(final Collection<? extends E> c) {
        this();
        addAll(c);
    }
     ...
     */
    private void addFirst(final E o) {
        checkNullArg(o);
     ...
     * @throws NullPointerException if the specified element is <tt>null</tt>
     */
    private void addLast(final E o) {
        checkNullArg(o);
     ...
     * @throws NullPointerException if the specified element is <tt>null</tt>
     */
    private boolean offerLast(final E o) {
        addLast(o);
        return true;
    }
     ...
     * @throws NullPointerException if the specified element is <tt>null</tt>
     */
    private boolean removeFirstOccurrence(final Object o) {
        checkNullArg(o);
        for (;;) {
        ...
        for (;;) {
            ...
            for (;;) {
                ...
                if (s.isDeleted() || ((n != null) && (n.successor() != s))) {
                    // restart if pred link is suspect.
                    break;
                }
                if (n == null) {
                    return false;
                } else if (o.equals(n.element)) {
                    if (n.delete()) {
                        return true;
                    } else {
                        // restart if interference
    ...
    /**
     * Creates an array list and fills it with elements of this list in reverse
     * order. Used by toArray.
     *
     * @return the arrayList
     */
    public List<E> toReverseArrayList() {
        final ArrayList<E> c = new ArrayList<>();
        for (Node<E> n = this.trailer.back(); n != null; n = n.back()) {
            c.add(n.element);
        }
        return c;
    }
 ...
 */
final class Node<E extends Serializable> extends AtomicReference<Node<E>> {
    /**
     * 
     */
    private static final long serialVersionUID = -4549623599993206206L;
    ...
    Node<E> append(final E element) {
        for (;;) {
            final Node<E> f = getNext();
            if ((f == null) || f.isMarker()) {
                return null;
            }
            final Node<E> x = new Node<>(element, f, this);
            if (casNext(f, x)) {
                // optimistically link
                f.setPrev(x);
                return x;
    ...
    Node<E> prepend(final E element) {
        for (;;) {
            final Node<E> b = predecessor();
            if (b == null) {
                return null;
            }
            final Node<E> x = new Node<>(element, this, b);
            if (b.casNext(this, x)) {
                // optimistically link
                setPrev(x);
                return x;
    ...
    Node<E> replace(final E newElement) {
        for (;;) {
            ...
            final Node<E> x = new Node<>(newElement, f, b);
    ...
    Node<E> successor() {
        ...
        for (;;) {
            ...
            final Node<E> s = f.nextNonmarker();
            if (f == getNext()) {
                // unlink f
                casNext(f, s);
            }
